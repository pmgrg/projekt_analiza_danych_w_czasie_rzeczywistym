        ...
        try:
            if firstline is True:
                ...
                if line[4] == "":
                    ...
                    time.sleep(3)
                    ID = int(line[0])
                    Year_Birth = int(line[1])
                    Education = str(line[2])
                    Marital_Status = str(line[3])
                    Income = int(line[4])
                    Kidhome = int(line[5])
                    Teenhome = int(line[6])
                    Dt_Customer = str(line[7])
                    Recency = int(line[8])
                    MntWines = int(line[9])
                    MntFruits = int(line[10])
                    MntMeatProducts = int(line[11])
                    MntFishProducts = int(line[12])
                    MntSweetProducts = int(line[13])
                    MntGoldProds = int(line[14])
                    NumDealsPurchases = int(line[15])
                    NumWebPurchases = int(line[16])
                    NumCatalogPurchases = int(line[17])
                    NumStorePurchases = int(line[18])
                    NumWebVisitsMonth = int(line[19])
                    AcceptedCmp3 = int(line[20])
                    AcceptedCmp4 = int(line[21])
                    AcceptedCmp5 = int(line[22])
                    AcceptedCmp1 = int(line[23])
                    AcceptedCmp2 = int(line[24])
                    Complain = int(line[25])
                    Z_CostContact = int(line[26])
                    Z_Revenue = int(line[27])
                    Response = int(line[28])
                    result = {}
                    result["ID"] = ID
                    result["Year_Birth"] = Year_Birth
                    result["Education"] = Education
